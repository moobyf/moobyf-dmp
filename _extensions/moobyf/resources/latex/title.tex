...
\large
$for(by-author)$
$it.name.literal$$for(it.affiliations)$\textsuperscript{$it.number$}$sep$,$endfor$$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$if(it.email)$~\href{mailto:$it.email$}{\faEnvelope}$endif$$sep$,  
$endfor$
...
\footnotesize
$for(affiliations)$
\textsuperscript{$it.number$} $it.name$\par
$endfor$
}
